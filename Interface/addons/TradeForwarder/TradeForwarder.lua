...
TFW_CHANNEL_NAME="TCForwarder6IIll";
...
local LFW_REALM = string.gsub(GetRealmName(), "%s", "");

local chatInfo = LFW_CHAT_INFO_DEFAULT;
tfwChNumber, channelName, instanceID = GetChannelName(TFW_CHANNEL_NAME);
local isInLFG = nil;
local isOwner = nil;
local currOwner = nil;
local msgIdx = 0;
...
DEFAULT_CHAT_FRAME:AddMessage(string.format(L["BIG_UPDATE"], sender, version, TFW_VERSION))
	...
	for i=1, NUM_CHAT_WINDOWS do
		...
		if show then
			...
			if ( filters and not WIM and not MrtWoo) then
				local filter = false;
				--local message, sender, language, channelString, target, flags, channelId, channelNumber, channelName, _, counter, guid = ...;
				local arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12 = msg, sender, DEFAULT_CHAT_FRAME.defaultLanguage, (tfwChNumber or 0)..". "..TFW_CHANNEL_NAME, "", "", 0, tfwChNumber, TFW_CHANNEL_NAME, "", counter or fake_counter, ""
...
local function SendCmd(cmd, flag)
        local tfwChNumber, channelName, instanceID = GetChannelName(TFW_CHANNEL_NAME);
        LFW_DEBUG("CHECK SEND CMD "..tfwChNumber..channelName..instanceID);
	if(tfwChNumber) then		
		local restore_traditionalize = false;
                 LFW_DEBUG("SEND CMD "..tfwChNumber);
...
local function CheckJoinedChannel()
        tfwChNumber, channelName, instanceID = GetChannelName(TFW_CHANNEL_NAME);
        LFW_DEBUG("test: "..tfwChNumber);
	isInLFG = false
	local channels = {GetChannelList()}
	for i=0, #channels, 2 do
...
local IgM_realm = LFW_REALM .. "-" .. UnitFactionGroup("player")
...
local function LFW_OnEvent(self,event,...) 
	...
	elseif(event=="CHAT_MSG_CHANNEL_NOTICE") then
		local message, sender, language, channelString, target, flags, _, channelNumber, channelName, _, _, _, counter = ...;
                tfwChNumber = channelNumber;
                CheckJoinedChannel();
	...
	elseif(event=="CHAT_MSG_CHANNEL_NOTICE_USER") then  		--Response to DisplayChannelOwner(TFW_CHANNEL_NAME);
		...
		if strfind(sender, LFW_REALM) then sender = strsplit("-", sender, 2) end
