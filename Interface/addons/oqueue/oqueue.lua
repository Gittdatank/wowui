...
function oq.hook_options()
  ...
  oq.options[ "doll"        ] = oq.paperdoll_check ;
...
function oq.is_raid()
  if (oq.raid.type == OQ.TYPE_RBG) or (oq.raid.type == OQ.TYPE_RAID)  or (oq.raid.type == OQ.TYPE_ROLEPLAY) then
    return true ;
  end
  return nil ;
end
...
function oq.auto_set_role()
  if (oq.toon.auto_role == 0) or (InCombatLockdown()) then
    return ;
  end
...
function oq.calc_pkt_stats()
  ...
  if (#oq.send_q > 20) then  -- more then 1 sec of pkts in the queue
    oq.tab5._oq_send_queuesz:SetText( string.format("(|cFFFF3131%d|r)", #oq.send_q ) ) ;
  elseif (#oq.send_q > 5) then
    oq.tab5._oq_send_queuesz:SetText( string.format("(|cFFFFD331%d|r)", #oq.send_q ) ) ;
  else
...
function oq.send_my_premade_info()
  ...
  if (raid == nil) or ((raid.next_advert) and (raid.next_advert > now)) then
    return ;
  end
...
function oq.push_initial_raid_info()
  if (oq.raid.raid_token) and (oq.raid.raid_token ~= "") then
    ...
    oq.process_premade_info( oq.raid.raid_token, oq.encode_name( oq.raid.name ), player_faction, 
                             oq.raid.level_range, oq.raid.min_ilevel, oq.raid.min_resil, oq.raid.min_mmr,
                             enc_data, oq.encode_bg( oq.raid.bgs ), 1, 
                             1, oq.utc_time(), 0, 
                             0, oq.raid.has_pword, oq.raid.is_realm_specific, oq.raid.type, 
...
--  oq.set_charm( my_group, my_slot, OQ.ICON_SKULL ) ; -- initial charm puts 'skull' on leader
  ...
  -- push it locally
  oq.process_premade_info( oq.raid.raid_token, oq.raid.name, player_faction, oq.raid.level_range, oq.raid.min_ilevel, oq.raid.min_resil, oq.raid.min_mmr,
...
function oq.raid_announce_relay( m )
  ...
  elseif (oq.raid.type == OQ.TYPE_RBG) or (oq.raid.type == OQ.TYPE_RAID) or (oq.raid.type == OQ.TYPE_ROLEPLAY) then
...
function oq.boss_announce( msg )
  if (not oq.iam_raid_leader() and not oq.iam_party_leader()) then  
    if (oq.raid.type ~= OQ.TYPE_RBG) and (oq.raid.type ~= OQ.TYPE_RAID) and (oq.raid.type ~= OQ.TYPE_ROLEPLAY) then
      return ;
    end
  end
...
function oq.send_req_waitlist( raid_token, pword ) 
  ...
  if (in_party) then
    ...
    local enc_data = oq.encode_data( "abc123", player_name, player_realm, player_realid ) ;
    local pdata = oq.get_pdata(raid.type) ; -- include the raid.type to get specific pdata
    if (pdata == "") then
      pdata = "AAA" ;
    end
    oq.realid_msg( raid.leader, raid.leader_realm, raid.leader_rid, 
...
function oq.group_invite_slot( req_token, group_id, slot ) 
  ...
  if ((oq.raid.type ~= OQ.TYPE_RBG) and (oq.raid.type ~= OQ.TYPE_RAID) and (oq.raid.type ~= OQ.TYPE_ROLEPLAY) and (group_id ~= my_group)) then
    -- proxy_invite needed
  ...
  for i=1,8 do
    if ((oq.raid.group[i].member[1] == nil) or (oq.raid.group[i].member[1].name == nil) or (oq.raid.group[i].member[1].name == "-")) then
      oq.menu_add( "group ".. i, 
                   ...
                   req_token, 
                   nil, 
                   function(self,arg1,arg2) oq.invite_group_leader( arg2, arg1 ) ; end
                 ) ;
  ...
  for i=1,8 do
    ...
    if (slot) then
      oq.menu_add( "group ".. i, 
                   ...
                   req_token, 
                   nil, 
                   function(self,arg1,arg2) oq.group_invite_first_slot_in( arg2, arg1 ) ; end
                 ) ;
    end
  end
  oq.menu_show_aligned( button, 2, -25, button:GetWidth()+10 ) ;
...
function oq.on_premade_item_click( self, button )
  ...
  if (self.raid_token) then
    r = oq.premades[ self.raid_token ] ;
    if (r) then
      local url = string.format("http://%s.battle.net/wow/%s/character/%s/%s/simple", string.sub(GetCVar("realmList"),1,2), string.sub(GetLocale(),1,2), r.leader_realm, r.leader ) ;
...
function oq.on_waitlist_item_click( self, button )
  ...
  if (self.token) then
    r = oq.waitlist[ self.token ] ;
    if (r) then
      local url = string.format("http://us.battle.net/wow/en/character/%s/%s/simple", r.realm, r.name ) ;
      local dialog = StaticPopup_Show("OQ_ArmoryPopup", url ) ;
      if (dialog ~= nil) then
        dialog.data = url ;
        dialog.text:SetJustifyH("LEFT") ;
...
function oq.create_tab1_common( parent )
  ...
  -- lucky charms  
  y = y + 30 ;
  oq.tab1._lucky_charms      = oq.button( parent, 250, parent:GetHeight()-40, 100, 25, OQ.LUCKY_CHARMS, 
                                    function(self) oq.assign_lucky_charms() ; end ) ;
  oq.tab1._lucky_charms:Hide() ;
  -- ready check
  oq.tab1._readycheck_button = oq.button( parent, 350, parent:GetHeight()-40, 100, 25, OQ.READY_CHK, 
...
function oq.set_premade_type( t )
  oq.raid.type = t ;
  if (oq.iam_raid_leader()) then
    if (t == OQ.TYPE_RBG) or (t == OQ.TYPE_RAID) or (t == OQ.TYPE_MISC) or (t == OQ.TYPE_ROLEPLAY) then
      ConvertToRaid() ;
    else
      ConvertToParty() ;
    end
...
function oq.premade_row_show( p ) 
  ...
  oq.create_raid_listing( oq.tab2._list, p.__x or 10, p.__y, oq.tab2._list:GetWidth() - 2*23, 25, p.raid_token, p.type ) ;
...
function oq.create_helpbox( parent )
  ...
  f.hint_but = oq.button( f, x, f:GetHeight() - 45, 45, 25, L["hints"], function(self) self:GetParent():Hide() ; oq.hint_shade() ; end ) ;
...
function oq.on_premade( raid_tok, raid_name, premade_info, enc_data, bgs_, type_, pdata_, leader_xp_, subtype_, preferences_ )
  ...
  oq.process_premade_info( raid_tok, raid_name, faction, level_range, min_ilevel, min_resil, min_mmr,
                           ...
                           nwaiting, has_pword, is_realm_specific, type_, pdata_, leader_xp_, karma, preferences_ ) ;
...
function oq.process_premade_info( raid_tok, raid_name, faction, level_range, ilevel, resil, mmr, enc_data, 
                                  ...
                                  is_realm_specific, type_, pdata_, leader_xp_, karma_, preferences_ )
  if (oq.toon.disabled) then
    return ;
  end
  local  now = oq.utc_time() ;
...
function oq.qualified_class( spec_id )
  ...
  local class = s.n:sub(1,2) ;
  local voip_, role_, classes_, lang_ = oq.decode_preferences( oq.raid._preferences ) ;
  if (oq.is_set( classes_, OQ.CLASS_FLAG[class] )) then
    return true ;
  end
  return nil ;
end
...
function oq.on_req_invite( raid_token, raid_type, n_members_, req_token, enc_data, stats, pword, pdata )
  ...
  elseif (not oq.qualified_role( m.spec_id )) then
    ...
    tbl.delete( m ) ;
    return ;
  elseif (not oq.qualified_class( m.spec_id )) then
...
function oq.send_xrealm( m ) 
  oq.bnfriends_relay( m, true, true ) ;
end
...
function oq.on_addon_event( prefix, msg, channel, sender )
  ...
  if ((channel == "WHISPER") and oq.iam_party_leader() and (sender == oq.raid.leader) and (not oq.toon.disabled)) then
...
function oq.check_seat( name, group_id, slot )
  ...
  return (oq.raid.group[group_id].member[slot].name == name) ;
end
...
function oq.on_party_members_changed()
  ...
  if (oq.iam_party_leader()) then
    ...
    if (oq.GetNumPartyMembers() == 2) and (oq.iam_raid_leader()) then
      ...
      if (oq.raid.type == OQ.TYPE_RBG) or (oq.raid.type == OQ.TYPE_RAID) or (oq.raid.type == OQ.TYPE_MISC) or (oq.raid.type == OQ.TYPE_ROLEPLAY) then
        ConvertToRaid() ;
      else
        ConvertToParty() ;
      end
  ...
  if (oq.iam_raid_leader() and ((oq.raid.type == OQ.TYPE_RBG) or (oq.raid.type == OQ.TYPE_RAID) or (oq.raid.type == OQ.TYPE_MISC) or (oq.raid.type == OQ.TYPE_ROLEPLAY))) then
    -- re-assign slots
    oq.assign_raid_seats() ;
  end
  if (my_slot == 1) then
...
function oq.on_unit_died( ... )
  ...
  if (OQ._unit_type[type] == "npc") or (OQ._unit_type[type] == "vehicle") then -- some bosses are 'vehicles'?
    ...
    if oq.is_boss( mobid ) and ((oq._last_boss_kill == nil) or ((oq._last_boss_kill ~= mobid) and (oq._2nd_to_last_boss_kill ~= mobid))) then
...
--  oq.ui:RegisterEvent("UPDATE_INSTANCE_INFO") ;
...
function oq.paperdoll_update( frame, target )
  ...
  for i,p in pairs( f._ilevels ) do
    ...
    p._ilink = ilink ;
    n, g1, g2, g3, g4, c1, c2, c3, c4, e_id, enchant_text, ilevel, is_pvp = oq.nGems( target, i ) ;
...
function oq.paperdoll_check( tag )
