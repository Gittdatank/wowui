...
function PWBIcon:AddIcon(ability, id)
	...
	self.iconCooldown[id] = CreateFrame("Cooldown", nil ,self.iconFrames[id], "CooldownFrameTemplate")
	self.iconCooldown[id]:SetAllPoints(self.iconFrames[id])
...
function PWBIcon:UpdateRage(rage)
	...
	for index, id in pairs(self.activeRageIcons) do
		...
		if isUsable then
			self.iconFrames[id].texture:SetAlpha(1)
		else
			self.iconFrames[id].texture:SetAlpha(self.iconList[id].lowRageAlpha)
		end
	end
end
...
function PWBIcon:UpdateDebufIcons()
	for index, id in pairs(self.activeDebuffIcons) do
		...
		if n then
			...
			if d > 0 then
				CooldownFrame_SetTimer(self.iconCooldown[id], e-d, d, 1)
			end
		else
			CooldownFrame_SetTimer(self.iconCooldown[id], 1, 1, 1)
		end
	end
end