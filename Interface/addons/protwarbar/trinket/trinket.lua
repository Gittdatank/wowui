...
function PWBTrinket:OnInitialize()
	...
	self.trinketFrames[1] = self:createIcon()
	self.trinketFrames[2] = self:createIcon()

	self.trinketCooldown[1] = CreateFrame("Cooldown", nil ,self.trinketFrames[1], "CooldownFrameTemplate")
	self.trinketCooldown[1]:SetAllPoints(self.trinketFrames[1])

	self.trinketCooldown[2] = CreateFrame("Cooldown", nil ,self.trinketFrames[2], "CooldownFrameTemplate")
	self.trinketCooldown[2]:SetAllPoints(self.trinketFrames[2])
end
...
function PWBTrinket:Update()
	for index, id in pairs(self.activeTrinkets) do
		self:UpdateCooldown(id, self.trinketFrames[index], self.trinketCooldown[index], self.cdTable[id])
	end
end
