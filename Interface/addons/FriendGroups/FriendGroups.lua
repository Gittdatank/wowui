            ...
            elseif ( FriendButtons[index].buttonType == FRIENDS_BUTTON_TYPE_BNET ) then
				...
				if ( characterName ) then
					if ( client == BNET_CLIENT_WOW and CanCooperateWithToon(toonID, hasTravelPass) ) then
						local colour = FriendGroups_SavedVars.colour_classes and ClassColourCode(select(8, BNGetToonInfo(toonID))) or FRIENDS_WOW_NAME_COLOR_CODE
...
local function FriendGroups_Update()
    --check oqueue broadcasts
	if FriendGroups_SavedVars.enable_oqueue then
...
local menu_items = {
	...
	[2] = {
		...
		{ text = "Enable oQueue grouping", checked = function() return FriendGroups_SavedVars.enable_oqueue end, func = function() CloseDropDownMenus() FriendGroups_SavedVars.enable_oqueue = not FriendGroups_SavedVars.enable_oqueue FriendGroups_Update() end },
	},
}
